...
class List{
    ...
    public:
        explicit List(){first = new elem; end = new elem; first->next = end; end->pre = first;}; //empty list
        ~List(){
            elem* temp = first;
        ...
        bool isEmpty() const{return empty;}; //returns if list is empty(no user data)
        bool isFull() const{return false;}; //always falls, linked list has no limits
...
template <typename T>
T& List<T>::operator[](int index){return (get(index).val);};
template <typename T>
void func(T& input){
    input*=2;
}
